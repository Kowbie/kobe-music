...
//----------------------------------- = [ Mute + Deafen ] = -----------------------------------\\
  ...
  let ch = oldM.guild.channels.find('name', 'glog')
  if(!ch) return;
...
client.on('messageUpdate', (message, newMessage) => {
    ...
    const channel = message.guild.channels.find('name', 'glog');
    if (!channel) return;
    ...
    role.guild.fetchAuditLogs({
      ...
      .then(audit => {
        ...
        try {
          ...
          let log = role.guild.channels.find('name', 'glog');
          if (!log) return;
          let embed = new Discord.RichEmbed()
  ...
  client.setTimeout(() => {
    guild.fetchAuditLogs({
      ...
      .then(audit => {
        let exec = audit.entries.map(a => a.executor.username);
        try {
          let log = guild.channels.find('name', 'glog');
          if (!log) return;
...
╚[❖════════════❖]╝
 ...
 ❖  Gkick <mention > ➾ لطرد عضو
 ❖  Gban لطرد عضو للابد
 ❖ Gmute < mention > ➾ اسكات عضو 
 ❖ Gvkick لطرد عضو من الروم الصوتي
 ❖ Gunmute <mention> ➾ فك الاسكات من العضو
 ❖ Gclear  ➾ لتنضيف المحادثة (fixing)
 ❖ Gbc1 الرسالة
 ❖ Gbc <message> ➾ لارسال رسالة لجميع اعضاء السيرفر
╔[❖════════════❖]╗
                   للمساعدة
╚[❖════════════❖]╝
...
『 Gplay ➾ لتشغيل أغنية برآبط أو بأسم』
『 Gskipتجآوز الأغنية الحآلية』
『 Gpa ➾ إيقآف الأغنية مؤقتا』
『 Gr➾ لموآصلة الإغنية بعد إيقآفهآ مؤقتا』
『 Gvol ➾ لتغيير درجة الصوت 100 - 0』
『 Gstop ➾ لإخرآج البوت من الروم』
『 Gnp ➾ لمعرفة الأغنية المشغلة حآليا』
『 Gq ➾ لمعرفة قآئمة التشغيل』
==================================================================
`);
